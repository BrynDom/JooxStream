...
SUPPORT_CHAT = "MusicStreamSupport"

@app.on_message(filters.command("wish"))
async def wish(_, m):
    if len(m.command) < 2:
        await m.reply("Tambahkan keinginan")
        return 
    ...
    wish = f"Hallo! {m.from_user.first_name}! "
    wish += f"Keinginan Anda {text} "
    wish += f"Mungkin untuk {wish_count}%"
